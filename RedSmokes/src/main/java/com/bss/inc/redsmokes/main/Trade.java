...
public class Trade {
    ...
    public BigDecimal getCommandCost(final IUser user) {
        ...
        if (command != null && !command.isEmpty()) {
            cost = redSmokes.getSettings().getCommandCost(command.charAt(0) == '/' ? command.substring(1) : command);
            if (cost.signum() == 0 && fallbackTrade != null) {
                cost = fallbackTrade.getCommandCost(user);
            }

            if (redSmokes.getSettings().isDebug()) {
                redSmokes.getLogger().log(Level.INFO, "calculated command (" + command + ") cost for " + user.getName() + " as " + cost);
            }
        }
